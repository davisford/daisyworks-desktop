...
public class STK500v1
{
   ...
   * -------------------------------------------------------------------------
   * devicecode     | Device code as defined in “devices.h”
   * ------------------------------------------------------------------------
   ...
   * -------------------------------------------------------------------------
   * flashpollval2  | FLASH polling value. Same as “flashpollval1”
   * -------------------------------------------------------------------------
  ...
  {
    final int shiftedAddress = address >> 1;
  ...
  public void writeProgram(final IntelHEXReader reader,
                           final long filesize)
    throws IOException
  {
    ...
    while (null != (chunk = reader.readNextChunk()))
    {
      if (chunks == 0)
      {
      ...
      System.arraycopy(chunk.data, 0, buf, 4 + bufSize, chunk.data.length);
      chunks++;

      bufSize += chunk.data.length;
    ...
    if (chunks > 0)
    {
      ...
      loadAddress(address);
      setupProgramPage();
      buf[2] = (byte)bufSize; // data block size (low)
      writeStart = System.currentTimeMillis();
  ...
  public void updateFirmware(final IntelHEXReader reader,
                             final long filesize)
    throws IOException
  {
    sync();
